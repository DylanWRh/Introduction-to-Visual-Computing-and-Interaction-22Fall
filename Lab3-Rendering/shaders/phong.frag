...
vec3 Shade(vec3 lightIntensity, vec3 lightDir, vec3 normal, vec3 viewDir, vec3 diffuseColor, vec3 specularColor, float shininess) {
    ...
    if (u_UseBlinn){
        specularLight = specularColor * lightIntensity * pow(max(dot(normalize(viewDir + lightDir), normal), 0), shininess);
    }
    else {
        specularLight = specularColor * lightIntensity * pow(max(dot(reflect(-lightDir, normal), viewDir), 0), shininess);
    }
    return diffuseColor * lightIntensity * max(dot(normal, lightDir), 0.) + specularLight;
}
